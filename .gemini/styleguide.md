...
- POST/PUT/DELETE → 생성된 함수를 `useMutation`으로 래핑 (orval이 POST를 `useQuery`로 잘못 생성하는 경우 있음)
...
- [ ] orval 생성 파일(`src/shared/api/generated/`)을 직접 수정하지 않았는가?
- [ ] POST/PUT/DELETE는 `useMutation`으로 래핑하고 있는가? (orval이 생성한 함수를 `useQuery`로 사용하지 않는가?)
